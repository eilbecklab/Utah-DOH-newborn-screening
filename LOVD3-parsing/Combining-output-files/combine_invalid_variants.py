...
args = ArgumentParser('./combine_invalid_variants.py', description='''This program is a simple one to combine the output files containing the
invalid variants (anything that failed HGVS normalization) into one csv file. Example usage:
./combine_invalid_variants.py --config_file LOVD3_Databases.json --LOVD3_directory output_files
--disease_names SCID Metabolic_Diseases --output_directory output_files''')

args.add_argument(
	'-c',
	'--config_file',
	#type=FileType('r'),
	help="""This is the config file that was used for the LOVD3 parser. If this option is not provided,
	the program will look for a json file in your present working directory.""",
	default = None,
)
...
args.add_argument(
	'-l',
	'--LOVD3_directory',
	help="""This is the name of the directory where the variants from LOVD3 parser are store d. The final output file will be stored
	in this directory if no output directory is stored with the option --output_directory. Default is output_files.""",
...
args.add_argument(
	...
	'--disease_names',
	nargs='+',
	help="""This is the same list of disease names supplied into LOVD3_Variant_Parser.py. If this is not specified, the program will
	try to gather the information from the directory specified with --LOVD3_directory.""",
	default = None
)
...
columns = ['Genome Assembly', 'Chr', 'Position Start', 'Position Stop', 'Ref', 'Alt',
			...
			'South_Asian_MAF', 'Other_Ancestry_MAF', 'HGVS Normalization Failure Reason']
