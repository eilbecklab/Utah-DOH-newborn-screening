...
args = ArgumentParser('./combine_variant_files_ClinVar.py', description='This program is a simple one to combine the output files of each group of variants (invalid HGVS, no star variants, variants with stars) into a single csv file. Example usage: ./combine_variant_files_ClinVar.py --ClinVar_directory output_files --disease_names SCID Metabolic_Diseases --output_directory output_files')

args.add_argument(
	'--ClinVar_directory',
	help="This is the name of the directory where the variants from the ClinVar parser are stored. The final output file will be stored in this directory if no output directory is stored with the option --output_directory. Default is output_files.",
...
args.add_argument(
	'--disease_names',
	nargs='+',
	help="This is the same list of disease names supplied into ClinVar_Parser.py. If this is not specified, the program will try to gather the information from the directory specified with --ClinVar_directory. ",
	default = None
)
...
columns_Invalid = ['Genome Assembly', 'Chr', 'Position Start', 'Position Stop', 'Ref', 'Alt',
			...
			'Submitter', 'Edited Date', 'Transcript Normalization Failure Message',
